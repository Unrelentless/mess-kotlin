...
class HeartBlockEntity: BlockEntity(ENTITY_TYPE), ExtendedScreenHandlerFactory {
    ...
    override fun writeScreenOpeningData(serverPlayerEntity: ServerPlayerEntity?, packetByteBuf: PacketByteBuf?) {
        val sortedLimbs = limbs?.sortedBy{it.inventory.isEmpty}
        val sizes = sortedLimbs?.map{it.level.size}?.toIntArray()
        val compoundTag = CompoundTag()
        val listTag = ListTag()
        sortedLimbs?.map { it.inventory.getStack().serializeInnerStackToTag() }
